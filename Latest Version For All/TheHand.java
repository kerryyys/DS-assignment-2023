    ...
    public static List<EdgeTheHand> prim(String graphStr, String rootVertex) {
        ...
        while (visited.size() < getTotalVertices(edges)) {
            ...
            for (EdgeTheHand edge : edges) {
                boolean isConnectedToVisited = visited.contains(edge.vertex1) ^ visited.contains(edge.vertex2);
                // xor to see connected to either vertex
            ...
            if (minEdge != null) {
                visited.add(visited.contains(minEdge.vertex1) ? minEdge.vertex2 : minEdge.vertex1);
                // checks which vertex is already visisted or not
                prims.add(minEdge);
            }
        }
